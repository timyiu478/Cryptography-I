...
fn main() {
    ...
    let target_cipher = hex::decode(&target_cipher_text[0..target_cipher_text.len()-1]).unwrap();

    let mut xor_map: HashMap<XorPair, Vec<u8>> = HashMap::new();
    let mut az_counter_map: HashMap<usize, Vec<usize>> = HashMap::new();


    // init az_counter_map
    for i in 0..cipher_texts.len() {
        let az_counter:Vec<usize> = vec![0; target_cipher.len()]; 
    ...
    for k in 0..target_cipher.len() {
        ...
        if max_count_index < cipher_texts.len() {
            ...
            target_plain_text.push(target_cipher[k] ^ target_cipher[k]);
        }
    }
